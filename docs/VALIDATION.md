...
- Recognized active checkout paths are intentionally fail-open/inactive. Returns remains suppressor-active, with known Rufus-powered return-workflow components explicitly preserved.
...
A dedicated live-Amazon Chromium acceptance suite covers homepage, search, product, cart, direct product startup, external→Amazon navigation, Amazon history Back/Forward, resize, two-tab toggle propagation, a fresh page while saved Off, public account/orders routes, checkout inactivity, Returns compatibility, dynamic mutation checks when Rufus is exposed by the runner, popup light/dark behavior, and a live-tab stability interval.
...
The same acceptance suite was then rerun against the final 1.0.0 release tree. After correcting only the test-harness selector from retired `.toggle-row` markup to current `.control-card`, the final-tree run passed **every check**: popup light/dark, homepage, search, product, cart, direct product, external→Amazon, Back/Forward, resize, two-tab toggle, saved-Off fresh tab, dynamic mutation handling, public account/orders routes, checkout inactivity, then-current Returns behavior, and live-tab stability. The final-tree rerun used a shorter live-tab interval because 30-minute stability had already been established and the release-tree runtime delta was separately covered by CI and confirmation-route tests.

## Returns workflow regression evidence

Signed-in Amazon US testing later showed that Amazon now reuses Rufus components inside the functional Returns prompt itself. The extension's original Returns safeguards protected `rufus-web-*`, `orc-rufus-*`, Rufus text/submit controls, and known input slots, but two shopping-suggestion containers were still classified as unconditional document-start hides.
